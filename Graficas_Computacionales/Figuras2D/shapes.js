...
function createPacMan(gl)
{
    ...
    for(let degree = 30; degree <=330 ; degree+=0.1)
{

    radian = ((degree*Math.PI)/180)
    verts.push(.5*Math.cos(radian));
    verts.push(.5*Math.sin(radian));
    verts.push(0.0);
    nverts++;
}
    ...
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    let pacman = {buffer:vertexBuffer, vertSize:3, nVerts:nverts, primtype:gl.TRIANGLE_FAN};
    return pacman;
}  